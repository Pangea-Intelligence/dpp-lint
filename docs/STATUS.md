...
## Erledigt
- **v0.3.0-Features (alle drei Nutzerpfade):** GitHub Action (action.yml im Repo, CI-Dogfooding-Job testet pass- und fail-Pfad), `template <module>` (Starter-Payload + Pflichtfeld-Liste mit DIN-Kapiteln, fixtures/ jetzt im npm-Paket), `--report <file>` für lint und risk (selbst-enthaltenes HTML: Ampel, Findings-Tabellen, Severity-Badges, Provenienz, Audit-Disclaimer). 74 Tests.
- **Beispiel-Szenario:** `examples/brandt-foerdertechnik/` (fiktiver Mittelständler prüft Zulieferdaten; 4 eingebaute Fehler, fixed-Variante, Origins mit CAHRA-Treffer). Im README verlinkt.
- **Upstream-Paket versandfertig (NICHT gesendet):** `docs/upstream/` mit PR-Text (E-Mail-Regex #54, Fix-Branch liegt im Org-Fork `Pangea-Intelligence/BatteryPassDataModel:fix/gpi-email-regex`) + 3 Issue-Texten (CC-BY-NC-Boilerplate 2 Varianten, warrentyPeriod=gMonth, Minor-Findings) + gh-Kommandos. Versand nur nach Felix-Review.
- **Marketing-Artefakte in pangea-analytix:** Erklär-Deck `docs/dpp-lint-praesi.pptx` (Script `scripts/build_dpplint_deck.py`), LinkedIn-Demo-Video 4:5 (`public/social/dpp-lint-demo.html` + `video/capture_dpplint.cjs`, Render in ~/Downloads), Launch-Plan `docs/launch-plan.md`.
