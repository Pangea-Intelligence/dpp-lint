...
function emptyDataset() {
  return {
    meta: {
      ...
      sources: [
        { name: "RMI Public List (all assessed facilities, incl. cobalt/lithium/nickel)", url: PUBLIC_LIST_PAGE },
...
async function fetchPublicList(allMetals) {
  ...
  try {
    ...
    console.error(`warning: could not read ${PUBLIC_LIST_PAGE} (${err.message}), using fallback viewer URL`);
  }
  const html = await fetchText(viewerUrl, { Referer: RMI_BASE + "/" });
  const theadMatch = html.match(/<thead[^>]*>([\s\S]*?)<\/thead>/);
  const tbodyMatch = html.match(/<tbody[^>]*>([\s\S]*?)<\/tbody>/);
  if (!theadMatch || !tbodyMatch) throw new Error("public list viewer: table not found (layout changed?)");
...
async function fetchCobaltList({ url, status }) {
  ...
  for (let pageNo = 1; pageNo <= 20; pageNo++) {
    ...
    const rows = [...html.matchAll(/<tr[^>]*class="cbResultSetDataRow[^"]*"[^>]*>([\s\S]*?)<\/tr>/g)];
    ...
    for (const [, rowHtml] of rows) {
      ...
      facilities.push({ name, rmiId, metal: "Cobalt", country, status, source: "rmi-cobalt-refiners-list" });
    }
...
sources.push({
  ...
  note: allMetals ? "all metals" : "filtered to battery raw materials (cobalt, lithium, nickel, natural graphite)",
});
...
for (const cfg of COBALT_PAGES) {
  try {
    ...
    sources.push({ name: `RMI Cobalt Refiners (${cfg.status})`, page: cfg.url, dataUrl: res.caspioBase, records: res.facilities.length });
    facilities.push(...res.facilities);
  } catch (err) {
    console.error(`warning: ${cfg.url} failed: ${err.message}`);
    sources.push({ name: `RMI Cobalt Refiners (${cfg.status})`, page: cfg.url, error: err.message });
  }
}
